...
class DialogXyz(QtWidgets.QDialog):
  ...
  def receiveRequestToView(self, strXmlRequest):
    if verboseEvent: 
      logger.info("%s %s receiveRequestToModel virtual method" % (self.__class.__name__, self.objectName()))
    return True
...
class DialogXmlXyz(QtWidgets.QDialog):
  ...
  def receiveRequestToView(self, strXmlRequest):
    if verboseEvent: 
      logger.info("%s %s receiveRequestToModel method virtual" % (self.__class.__name__, self.objectName()))
    return True
