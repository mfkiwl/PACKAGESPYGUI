...
"""
  ...
  def test_300(self):
    if PACKAGESPY_ROOT_DIR is None:
      print("\nPACKAGESPY_ROOT_DIR is None, skip test_300 ekinoxpy")
      return
    from ekinoxpy.ekinoxEki import EkinoxEki as DEKI
    app = OnceQApplication()
    dirs = glob.glob(os.path.join(testDir, "ekinoxSim_test??"))
    self.assertNotEqual(dirs, []) #empty list is not a test
    f = dirs[0]
    v = False
    aData = DEKI()
    aData.setFromDirectoryEki(f, v)
    alliageData = aData.AlliageEki
    data = alliageData.toStrXml()
    if verbose: print("test dialog on: \n",data)
    wid = DialogXmlXyz()
    wid.setFromXml(data)
