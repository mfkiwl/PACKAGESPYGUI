...
class QActionXyz(QtWidgets.QAction):
  ...
  def setAction(self, slot=None, signal=None, shortcut=None, tooltip=None, icon=None):
    ...
    if self.slot != None:
      logger.warning("slot of common signal %s is affected yet", self.name)
      return False
  ...
  def defaultSlot(self, args=None):
    ...
    #sender = self.sender()
    logger.warning("default slot of action of common name %s only for test, args : %s", self.name, args)
    self.lastDefaultSlotArgs.append(args)
    
  def createNewAction(self, name=None, slot=None, text=None, toolTip=None):
    ...
    logger.info("createNewAction from action %s",self.name)
    if name != None:
      action.name = name
      action.setText(name)
    if text != None:
...
def addInCommonActions(action):
  if action.name == None:
    logger.error("problem action without name : %s" % action.name)
    return False
  try:
    if type(action.name) != str:
      logger.warning("name is not type str : %s", type(action.name))
      return False
    if action.name in list(__commonActions__.keys()):
      #print __commonActions__
      if os.getenv("USERNAME") in ["christian", "wambeke"]:
        #mess = "existing yet action '%s'" % action.name
        mess = "existing yet action with name '%s' (to check if not unittest)" % action.name
        if mess not in _messDone and not _inUnittest: #avoid message warning in unittest
          logger.warning(mess)
          _messDone.append(mess)
      return False
    else:
       logger.debug("create action '%s'" % action.name)
       __commonActions__[action.name] = action
       return True
  except Exception as e:
    logger.error("problem with action : %s\n  %s" % (action.name, e))
    return False
