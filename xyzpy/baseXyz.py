...
class _BaseCommonXyz(object):
  ...
  def receiveRequestToModel(self, strXmlRequest):
    ...
    """
    if verbose:
      logger.info("%s.receiveRequestToModel (%s)\n%s" % (self._className, self.objectName(), strXmlRequest))
    elif verboseEvent: 
      logger.info("%s.receiveRequestToModel (%s)" % (self._className, self.objectName()))
    else:
      pass
    ...
    if "TestRequestFromView" in aRequest.typeRequest: #reply the request to views (for test)
      ...
      if not res: logger.warning("%s.receiveRequestToModel Problem in TestRequestFromView" % self._className)
    return True
